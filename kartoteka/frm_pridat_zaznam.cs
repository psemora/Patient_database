...
namespace kartoteka
{
    public partial class frm_pridat_zaznam : Form
    {
        OleDbConnection pripojeni;

        public frm_pridat_zaznam()
        {
            InitializeComponent();
        ...
        private void btn_uloz_zaznam_Click(object sender, EventArgs e)
        {
            String SqlPrikaz = "";
            OleDbCommand prikaz;
            OleDbDataAdapter adapter = new OleDbDataAdapter();
            SqlPrikaz = "INSERT INTO lecba (ID_pacienta, nemoc, priznaky, leky, kontrola, poznamka) VALUES(" + frm_kartoteka.ListView1.SelectedItems[0].Text.ToString() + ",'" + txt_nemoc.Text + "','" + txt_priznaky.Text + "','" + txt_leky.Text + "','" + dtp_kontrola.Value.ToString() + "','" + txt_poznamka.Text + "')";
            ...
            pripojeni.Close();
            this.Close();
            SQL_prikazy.ProvedSqlPrikaz2("SELECT lecba.ID_zaznamu, lecba.nemoc, lecba.priznaky, lecba.leky, lecba.kontrola, lecba.poznamka FROM lecba INNER JOIN pacienti ON lecba.id_pacienta = pacienti.id_pacienta WHERE pacienti.ID_pacienta = " + frm_kartoteka.ListView1.SelectedItems[0].Text.ToString());
            MessageBox.Show("Záznam byl úspěšně vložen", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
